            ...
            <div class="row  justify-content-center h-100">
                        ...
                        <div class="card-body contacts_body">
                            <ui class="contacts">
                                @foreach ($data_magangs as $data_magang)
                                    <a href="javascript:void()" wire:click="select('{{ $data_magang->id }}')"
                                            ...
                                            <div class="d-flex bd-highlight align-items-center">
                                                <div class="img_cont">
                                                    @if (auth()->user()->role == 'peserta')
                                                        <img src="
                                                            @if ($data_magang->pembimbing->img) {{ Storage::url($data_magang->pembimbing->img) }}
                                                                    @else
                                                    ...
                                                    @else
                                                        <img src="
                                                    @if ($data_magang->peserta->img) {{ Storage::url($data_magang->peserta->img) }}
                                                            @else
                                                            {{ asset('sneat/assets/img/avatars/1.png') }} @endif
                                                    "
                                                            class="rounded-circle user_img">
                                                ...
                                                <div class="user_info">
                                                    @if (auth()->user()->role == 'peserta')
                                                        ...
                                                        <span>{{ $data_magang->pembimbing->nama }}</span>
                                                    @else
                                                        <div class="" style="font-size: 14px">
                                                            <small>peserta</small>
                                                        </div>
                                                        <span>{{ $data_magang->peserta->nama }}</span>
                                                    @endif
                                                    <div
                                                        class="{{ $data_magang->status != 'magang' ? 'text-success' : 'text-info' }}">
                                                        <small>{{ $data_magang->status }}</small>
                                                    </div>
                                                    <div class="" style="font-size: 15px">
                                                        <small>{{ $data_magang->magang_mulai }} -
                                                            {{ $data_magang->magang_selesai }}</small>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                ...
                <div class="col-md-9 col-xl-7 chat ">
                    <div class="card ">
                            ...
                            <div class="d-flex bd-highlight">
                                @if ($data_magang_chat)
                                    @if (auth()->user()->role == 'peserta')
                                        <div class="img_cont">
                                            <img src="
                                            @if ($data_magang_chat->pembimbing->img) {{ Storage::url($data_magang_chat->pembimbing->img) }}
                                            @else
                                             {{ asset('sneat/assets/img/avatars/1.png') }} @endif
                                            "
                                                class="rounded-circle user_img">
                                        ...
                                        <div class="user_info">
                                            <span>
                                                {{ $data_magang_chat->pembimbing->nama }}

                                            </span>
                                            <p>{{ $data_magang_chat->chats->count() }} Messages</p>
                                        </div>
                                        <div class="video_cam">
                                    ...
                                    @if (auth()->user()->role == 'pembimbing')
                                        <div class="img_cont">
                                            <img src="
                                            @if ($data_magang_chat->peserta->img) {{ Storage::url($data_magang_chat->peserta->img) }}
                                            @else
                                             {{ asset('sneat/assets/img/avatars/1.png') }} @endif
                                            "
                                                class="rounded-circle user_img">
                                        ...
                                        <div class="user_info">
                                            <span>{{ $data_magang_chat->peserta->nama }}</span>
                                            <p>1767 Messages</p>
                                        </div>
                                        <div class="video_cam">
                                            {{-- <span><i class="fas fa-video"></i></span> --}}
                                ...
                                @else
                                    <div class="img_cont">
                                        <img src="{{ asset('sneat/assets/img/avatars/1.png') }}" class="rounded-circle user_img">
                                        <span class="online_icon"></span>
                                    </div>
                                    <div class="user_info">
                                        <span>Pilih terlebih dahulu</span>
                                        {{-- <p>1767 Messages</p> --}}
                                    </div>
                                @endif
                        ...
                        <div class="card-body msg_card_body" wire:poll.750ms
                                    ...
                                    @if ($chat->pengirim_id != auth()->user()->id)
                                        <div class="mb-4">
                                            ...
                                            <div class="d-flex justify-content-start ms-3">
                                                <small class=""
                                                    style="font-size: 11px">{{ \Carbon\Carbon::parse($chat->created_at)->isoFormat('dddd, D MMMM Y, HH:mm') }}</small>
                                            </div>
                                        </div>
                                    @endif
                                        ...
                                        <div class=" mb-4">
                                            <div class="d-flex justify-content-end ">
                                                <div class="msg_cotainer_send">
                                                    ...
                                                    @if ($chat->read_at)
                                                        <span class=""
                                                            style="font-size: 11px;color:dodgerblue"><svg
                            ...
                            @if ($data_magang_chat)
                                <form wire:submit.prevent='send'>
                                    ...
                                    <span class="input-group-text attach_btn"><i
                                        ...
                                        <input wire:model='pesan' class="form-control type_msg"
                                            placeholder="Type your message..."></input>
                                        <button type="submit" class="input-group-append btn btn-transparent">
...
<style>
    ...
    .card {
        /* height: 600px; */
        height: 80vh;
        /* Mengatur tinggi card menjadi 80% dari tinggi layar */
        /* width: 80%; */
        /* Mengatur lebar card */
        /* margin: auto; */
        /* Mengatur card agar berada di tengah */
